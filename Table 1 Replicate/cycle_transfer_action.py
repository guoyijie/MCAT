...
from models import ContextForwardModel, ImagePool, MultiPairAxmodel, ConditionDmodel, GANLoss
...
class ActionTransferAgent(object):
    ...
    def train_context_forward(self, writer=None, t=0):
        ...
        given_sim_param = np.array([], np.float32).reshape((0, 1))
        mse_losses, recon_losses, const_losses = [], [], []
        if self.env_name=='HalfCheetah-v2' and self.forward_loss < 0.02:
        ...
        for batch_num in range(forward_n_update):
            if np.random.rand() < 0.5:
                a_idx = np.random.randint(0, len(self.data_types))
                b_idx = a_idx
                label = 0
            else:
                a_idx, b_idx = np.random.choice(list(np.arange(len(self.data_types))), size=2, replace=False)
            ...
            if batch_num>forward_n_update-10:
                ...
                given_sim_param = np.concatenate((given_sim_param, a_sim_param), axis=0)
            if batch_num%100 == 0:
        ...
        # data=context_tsne_plot(learned_context, given_sim_param[:,0])
        # writer.add_image('context', data, t, dataformats='HWC')
    ...
    def train_transfer_agent_bisimilar(self, writer=None, t=0):
        ...
        pair_transfer_distance = [[[] for _ in self.data_types] for _ in self.data_types]
        target_context_tensor =None
        for i in range(self.cycle_n_update):
            source_index, target_index = np.random.choice(list(np.arange(len(self.data_types))), size=2)
            ...
            trans_action = self.model(source_state, source_action, mean_source_context_tensor, mean_target_context_tensor)
            ...
            #loss_cycle = nn.MSELoss()(normalized_delta, out_mu) 
            #loss_cycle = nn.L1Loss()(normalized_delta, out_mu)
            #loss_cycle = nn.L1Loss(reduction='none')(normalized_delta, out_mu)/out_logvar.exp().sqrt() 
            #loss_cycle = loss_cycle.mean()
            loss_cycle = nn.L1Loss()(normalized_delta, out_mu)
            #_, in_mu, in_logvar = self.mlp(source_state, source_action, source_context_tensor, norm_obs_mean, norm_obs_std, norm_act_mean, norm_act_std, norm_delta_mean, norm_delta_std, self.deterministic)
            #in_dist = torch.distributions.normal.Normal(in_mu.detach(), in_logvar.detach().exp().sqrt())
            #out_dist = torch.distributions.normal.Normal(out_mu, out_logvar.exp().sqrt()) 
            #loss_cycle = torch.distributions.kl_divergence(in_dist, out_dist).mean() 
            #loss_g_gan = self.cal_gan(target_action, trans_action, source_index, target_index, mean_source_context_tensor, mean_target_context_tensor)
    
            loss_all = loss_cycle#+loss_g_gan
    ...
    def train_transfer_agent(self, writer=None, t=0):
        (norm_obs_mean, norm_obs_std, norm_act_mean, norm_act_std, norm_delta_mean, norm_delta_std, norm_cp_obs_mean, norm_cp_obs_std, norm_cp_act_mean, norm_cp_act_std) = self.get_normalization_stats()
        ...
        for i in range(self.cycle_n_update):
            ...
            _, out, _ = self.mlp(source_state, trans_action, mean_target_context_tensor, norm_obs_mean, norm_obs_std, norm_act_mean, norm_act_std, norm_delta_mean, norm_delta_std, self.deterministic)
            out = denormalize(out, norm_delta_mean, norm_delta_std)
            delta = source_nxt_state-source_state
            loss_cycle = loss_fn(out, delta)*20
            loss = loss_cycle

            #back_action = self.back_model(source_state, trans_action, mean_source_context_tensor, mean_target_context_tensor)
            #loss_back = loss_fn(back_action, source_action)
            #loss += loss_back
            loss_back = torch.FloatTensor([0.0])

            #loss_g_gan = self.cal_gan(target_action, trans_action, source_index, target_index, mean_source_context_tensor, mean_target_context_tensor)
            loss_g_gan = torch.FloatTensor([0.0])
            ...
            epoch_loss.append(loss_cycle.item())
            cmp_loss.append(loss_back.item())
            gan_loss.append(loss_g_gan.item())
            if i%100 == 0:
                print('cycle_loss:{:.3f}  back_loss:{:.3f}  gan_loss:{:.3f}'.format(np.mean(epoch_loss), np.mean(cmp_loss), np.mean(gan_loss)))

        writer.add_scalar('cycle_loss', np.mean(epoch_loss), t)
        writer.add_scalar('back_loss', np.mean(cmp_loss), t)
        writer.add_scalar('gan_generator_loss', np.mean(gan_loss), t)


    def cal_gan(self, real_action, trans_action, source_index, target_index, source_context, target_context):
        ######################
        # (1) Update D network
        ######################
        ...
        fake_b = trans_action
        fake_ab = self.fake_pools[target_index].query(fake_b.detach())
        pred_fake = self.dmodel(fake_ab, target_context)
        loss_d_fake = self.criterionGAN(pred_fake, False)

        real_b = real_action
        real_ab = self.real_pools[target_index].query(real_b)
        pred_real = self.dmodel(real_ab, target_context)
        ...
        if not target_index==source_index:
            self.optimizer_d.zero_grad()

            pred_fake = self.dmodel(fake_b.detach(), source_context)
            loss_d_fake = self.criterionGAN(pred_fake, True)

            real_ab = self.real_pools[target_index].query(real_b)
            pred_real = self.dmodel(real_ab, source_context)
            loss_d_real = self.criterionGAN(pred_real, False)

            loss_d_target = (loss_d_fake + loss_d_real) * 0.5
            loss_d_target.backward()
            self.optimizer_d.step()

            loss_d += loss_d_target
            loss_d /= 2.0
    ...
    def evaluate(self, shared_policy, shared_policy_reward, nested_policy_reward, n_episodes=10, writer=None, t=0):
        ...
        shared_policy_reward = np.array([shared_policy_reward for _ in self.data_types]).transpose()
        improvement = (eval_transfer_rewards-shared_policy_reward)/np.maximum(np.abs(shared_policy_reward), 1)*100.0
        data = improvement_plot(improvement, row_label=self.data_types, col_label=self.data_types)
        writer.add_image('performance_drop', data, t+1, dataformats='HWC')

        improvement = (eval_transfer_rewards-np.array(nested_policy_reward))/np.maximum(np.abs(nested_policy_reward), 1)*100.0
        data = improvement_plot(improvement, row_label=self.data_types, col_label=self.data_types)
        writer.add_image('performance_improvement', data, t+1, dataformats='HWC')
    ...
    def online_test(self, idx, n_episodes, source_context_tensor=None, shared_policy=None):
        ...
        for episode in (range(n_episodes)):
            ...
            while not done:
                ...
                act = self.model(obs_tensor,good_action,source_context, target_context).cpu().data.numpy().flatten()
